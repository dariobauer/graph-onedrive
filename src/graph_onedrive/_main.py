...
class OneDrive:
    ...
    def __init__(self, client_id: str, client_secret: str, tenant: str = "common", redirect_url: str = "http://localhost:8080", refresh_token: Optional[str] = None) -> None:
    ...
    def _get_authorization(self) -> str:
        ...
        print("Following authorization you will be redirected to a browser error page - this is normal...")
        print("Step 3: On that error page, copy the response URL - click into the address bar and copy all.")
        response = input("Step 4: paste the response here: ")

        # Verify the state which ensures the response is for this request
        return_state = response[(response.find("state") + len("state") + 1) : (response.find("&session_state"))]
        if state != return_state:
            raise Exception("The response does not correspond to this original request.")

        # Extract the code from the response
        authorization_code = response[(response.find("code") + len("code") + 1) : (response.find("&state"))]
    ...
    def _create_headers(self) -> None:
        ...
        if len(self._access_token) < 1800:
            raise Exception("Invalid access token was provided (too short), try clearing it from the config.json file.")
        # Set headers
    ...
    def get_usage(self, unit: str = "gb", refresh: bool = False, verbose: bool = False) -> Tuple[float, float, str]:
        ...
        # Print usage
        if verbose:
            print(f"Using {used} {unit} ({round(used*100/capacity, 2)}%) of total {capacity} {unit}.")
    ...
    def detail_item(self, item_id: str, verbose: bool = False) -> Dict[str, Any]:
        ...
        if response.status_code != 200:
            print(response.text)
            raise Exception(f"API Error {response.status_code}: item could not be detailed.")
        response = json.loads(response.text)
        ...
        if verbose:
            ...
            if "folder" in response:
                ...
                print("type:", "file")
            print("created:", response["createdDateTime"], "by:", response["createdBy"]["user"]["displayName"])
            print("modified:", response["lastModifiedDateTime"], "by:", response["lastModifiedBy"]["user"]["displayName"])
    ...
    def make_folder(self, folder_name: str, parent_folder_id: Optional[str] = None, check_existing: bool = True, if_exists: str = "rename") -> str:
        ...
        if parent_folder_id:
            request_url = self._API_URL + "me/drive/items/" + parent_folder_id + "/children"
        else:
            request_url = self._API_URL + "me/drive/root/children"
        ...
        if if_exists == "fail":
            ...
            raise Exception("if_exists input value was not valid. Str type of values 'fail', 'replace', 'rename', are only accepted.")
    ...
    def move_item(self, item_id: str, new_folder_id: str, new_name: Optional[str] = None) -> Tuple[str, str]:
        ...
        if response.status_code != 200:
            print(response.text)
            raise Exception(f"API Error {response.status_code}: item could not be moved.")
        response = json.loads(response.text)
        item_id = response["id"]
    ...
    def copy_item(self, item_id: str, new_folder_id: str, new_name: Optional[str] = None, confirm_complete: bool = False) -> str:
        ...
        if confirm_complete:
            ...
            while True:
                ...
                wait_duration = 100.0 / (percentage_complete - previous_complete) * wait_duration - wait_duration
                if wait_duration > 30:
                    wait_duration = 30
    ...
    def rename_item(self, item_id: str, new_name: str) -> str:
        ...
        if response.status_code != 200:
            print(response.text)
            raise Exception(f"API Error {response.status_code}: item could not be renamed.")
        response = json.loads(response.text)
    ...
    def delete_item(self, item_id: str, pre_confirm: bool = False) -> bool:
        ...
        if not pre_confirm:
            confirm = input("Deleted files may not be restorable. Are you sure that you want to delete this file? [y/N]: ")
            if confirm != "y":
                print("Aborted.")
                return False
        ...
        if response.status_code != 204:
            print(response.text)
            raise Exception(f"API Error {response.status_code}: item could not be deleted.")
    ...
    def download_file(self, item_id: str) -> str:
        ...
        if "folder" in details:
            raise Exception("Item id provided is for a folder which this function does not permit.")
        # Create request url based on input item id to be downloaded
        request_url = self._API_URL + "me/drive/items/" + item_id + "/content"
        # Make the Graph API request
        response = requests.get(request_url, headers=self._headers, allow_redirects=False)
        # Validate request response and parse
        if response.status_code != 302:
            print(response.text)
            raise Exception(f"API Error {response.status_code}: item could not be downloaded.")
    ...
    @token_required
    def upload_file(self, file_path: Union[str, Path], new_file_name: Optional[str] = None, parent_folder_id: Optional[str] = None, if_exists: str = "rename") -> str:
        ...
        if file_size > basic_file_limit:
            ...
            response = self.upload_large_file(file_path, file_name, parent_folder_id, if_exists)
            return response
       # Create request url based on input values
        ...
        request_url += file_name + ":/content" + "?@microsoft.graph.conflictBehavior=" + conflict_behavior
        ...
        if response.status_code != 201 and response.status_code != 200:
            print(response.text)
            raise Exception(f"API Error {response.status_code}: could not upload file: {file_path}")
        response = json.loads(response.text)
        item_id = response["id"]
    ...
    @token_required
    def upload_large_file(self, file_path: Union[str, Path], new_file_name: Optional[str] = None, parent_folder_id: Optional[str] = None, if_exists: str = "rename") -> str:
        ...
        if if_exists == "fail":
            ...
            raise Exception(f"if_exists={if_exists} not valid. Str type of values 'fail', 'replace', 'rename', are only accepted.")
        ...
        try:
            ...
            while data.tell() < file_size:
                ...
                if n == 1:
                    ...
                    print(f"Uploading chunk {n}/{no_of_uploads}  (~{int((n-1)/no_of_uploads*100)}% complete)")
                # Upload chunks
                if ((file_size - data.tell()) > chunk_size):
                    ...
                    if response.status_code != 202:
                        ...
                        raise Exception(f"API Error {response.status_code}: could not upload chuck {n} of {no_of_uploads}")
